...
describe("emacs: dates", function()
  ...
  it("evaluates a date range", function()
    ...
    with_stub(vim, "notify", function(m)
      ...
      eq("2 days 3 hours 15 minutes", msg)
      eq("<2026-01-01 Thu 10:00>--<2026-01-03 Sat 13:15> => old", buf_lines(buf)[1])
      timestamps.evaluate_time_range(true)
    end)
    -- Emacs format; a result already there is replaced
    eq("<2026-01-01 Thu 10:00>--<2026-01-03 Sat 13:15> 2d 03:15 => old", buf_lines(buf)[1])
  ...
  it("evaluates a time range", function()
    ...
    with_stub(vim, "notify", function(m)
      ...
      timestamps.evaluate_time_range(false)
      eq("2 hours 30 minutes", msg)
      timestamps.evaluate_time_range(true)
    end)
    eq("Meet <2026-01-01 Thu 10:00-12:30> 02:30 ok", buf_lines(buf)[1])
  end)
...
describe("emacs: todo / notes / timer", function()
  ...
  it("insert_item adds a timer list item (org-timer-item)", function()
    local buf = org_buffer({ "  - first", "" }, { 1, 0 })
    silence(function()
      -- not a timer list: Emacs refuses
      with_stub(utils, "error", function() end, function()
        eq(nil, timer.insert_item())
      end)
      eq({ "  - first", "" }, buf_lines(buf))
      buf = org_buffer({ "" }, { 1, 0 })
      timer.insert_item()
      vim.cmd("stopinsert")
      eq("- 0:00:00 :: ", buf_lines(buf)[1])
      timer.insert_item()
      vim.cmd("stopinsert")
      timer.stop()
    end)
