...
describe("structure: keymaps integration", function()
  ...
  it("M-RET in Normal mode at column 0 of an item adds the item after it", function()
    local buf = org_buffer({ "- a", "  - sub", "- b" }, { 1, 0 })
    keys("<M-CR>")
    vim.cmd("stopinsert")
    eq({ "- a", "  - sub", "- ", "- b" }, buf_lines(buf))
    buf = org_buffer({ "- [ ] a" }, { 1, 0 })
    keys("<M-S-CR>")
    vim.cmd("stopinsert")
    eq({ "- [ ] a", "- [ ] " }, buf_lines(buf))
    -- Insert mode keeps Emacs point semantics: before the item
    buf = org_buffer({ "- a" }, { 1, 0 })
    keys("i<M-CR>")
    vim.cmd("stopinsert")
    eq({ "- ", "- a" }, buf_lines(buf))
