...
describe("agenda.search", function()
  ...
  it("properties", function()
    ...
    -- like Emacs, numbers compare with `string-to-number' ("1:30" is 1)
    eq({}, matches("Effort>60"))
    eq({ "Work task" }, matches("Effort>=1"))
  ...
  it("reports errors", function()
    ...
    eq(nil, pred)
    ok(err)
    -- like Emacs, text that does not parse ends the match silently
    eq({}, matches('FOO="unterminated'))
  end)
