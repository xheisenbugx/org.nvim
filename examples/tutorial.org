...
=content=, =showall=, =showeverything= or =nofold=. Without it, the
=startup_folded= option decides.
...
*Try:* press =<M-CR>= on "Fruit" below and type "Vegetables".
...
- =:Org align_tags= realigns every tag in the buffer to =tags_column=.
...
efforts of the children. In the view, =e= edits a value, =<CR>= jumps to
the entry, =r= refreshes and =q= quits.

*Try:* put the cursor on "Write the quarterly report" and press =<prefix>C=.

Summary operators: ={+}= sum, ={:}= time sum, ={X}= checkbox, ={X/}= and
={X%}= checkbox statistics, ={min}= ={max}= ={mean}=.
...
everything again and =<prefix>/= =c= to clear the highlights.
...
- =<prefix>if= inserts a new footnote and its definition.
