...
** Following links from a headline
...
#+begin_src python :wrap src json :cache yes
import json
...
#+begin_src sh :results output :file hello.txt
echo "Written by babel"
#+end_src

Inline blocks work too: put the cursor on src_lua{return 6 * 7} and press
=<C-c><C-c>=.
