...
--- Compile one term into predicate(hl, tags). With tag `groups`, a group
--- tag (`[ GTD : Control Persp ]` in #+TAGS) matches itself and its
--- members, recursively and case-insensitively, `{regexp}` members by
--- regexp (org-tags-expand).
---@param groups? table<string, string[]>
local function compile_term(t, groups)
  local f
  local expanded = t.kind == "tag" and groups and require("org.tags").expand_group(t.name, groups)
  if expanded then
    local res = {}
    for _, re in ipairs(expanded.regexps) do
      res[#res + 1] = M.compile_emacs_regexp(re)
    end
    f = function(_, tags)
      for tag in pairs(tags()) do
        if expanded.names[tag] then
          return true
        end
...
function M.compile(match, opts)
  ...
  if tag_part:match("%S") then
    ...
    while k <= #orterms do
      ...
      local preds = {}
      for _, t in ipairs(parse_and(term)) do
        preds[#preds + 1] = compile_term(t, tag_groups)
      end
      groups[#groups + 1] = preds
    end
...
function M.try_compile(match, opts)
  local ok, res = pcall(M.compile, match, opts)
  if not ok then
