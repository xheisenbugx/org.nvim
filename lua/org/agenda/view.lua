...
local function reset_filters()
  S.filters = { tags = { include = {}, exclude = {} }, category = nil, regexp = nil }
end
...
local function item_filter()
  local f = S.filters
  local active = next(f.tags.include) or next(f.tags.exclude) or f.category or f.regexp
  if not active then
    return nil
  end
